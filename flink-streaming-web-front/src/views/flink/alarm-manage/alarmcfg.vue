  ...
  <div class="tab-container">
    <el-form ref="form" :model="form" label-width="100px">
      <el-form-item inline="true">
        ...
        <el-input v-model="form.dingding_alarm_url" placeholder="钉钉告警所需的url" class="fl-form-item" />
        <el-button type="primary" @click="updateConfig('dingding_alarm_url', form.dingding_alarm_url)">提交</el-button>
        <el-button type="danger" @click="deleteConfig('dingding_alarm_url')">删除</el-button>
      ...
      <el-form-item inline="true">
        ...
        <el-input v-model="form.callback_alarm_url" placeholder="自定义http回调告警" class="fl-form-item" />
        <el-button type="primary" @click="updateConfig('callback_alarm_url', form.callback_alarm_url)">提交</el-button>
        <el-button type="danger" @click="deleteConfig('callback_alarm_url')">删除</el-button>
...
<script>
import { alarmConfig, upsertSynConfig, deleteConfig, test_alarm } from '@/api/config'
  ...
  methods: {
    queryConfig() { // 查询配置列表
      ...
      alarmConfig().then(response => {
        ...
        this.form.dingding_alarm_url = ''
        this.form.callback_alarm_url = ''
        data.forEach((item) => {
          if (item.key === 'dingding_alarm_url') {
            this.form.dingding_alarm_url = item.val
          } else if (item.key === 'callback_alarm_url') {
            this.form.callback_alarm_url = item.val
          }
        })
      }).catch(error => {
        this.loading = false
    ...
    deleteConfig(key) { // 删除配置
      let keyname = ''
      if (key === 'dingding_alarm_url') {
        keyname = '钉钉告警通知配置'
      } else if (key === 'callback_alarm_url') {
        keyname = '自定义告警通知配置'
      }
