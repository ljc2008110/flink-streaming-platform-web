    ...
    <el-table :data="list" :header-cell-style="{background:'#f4f4f5','text-align':'center'}" class="wl-table" border>
      ...
      <el-table-column prop="alarmLogStatusEnum" label="状态" width="65" align="center">
        <template slot-scope="scope">
          ...
          <el-tag v-else type="info" size="mini">{{ getAlarmLogStatus(scope.row.alarmLogStatusEnum) }}</el-tag>
        </template>
      </el-table-column>
...
<script>
import { alarmLogList, logErrorInfo } from '@/api/alert'
...
export default {
  ...
  methods: {
    ...
    getLogs() { // 查询日志列表
      ...
      alarmLogList(this.currentPage, this.pageSize, jobConfigId, status).then(response => {
        this.loading = false
