  ...
  <div v-loading="loading">
    <div v-if="subPageFlag==false" class="fl-container">
      ...
      <el-form ref="queryform" :model="queryform" :inline="true">
        ...
        <el-form-item>
          <el-select v-model="queryform.jobType" placeholder="任务类型" style="width: 120px;" clearable @change="handleQuery()">
        ...
        <el-form-item>
          <el-select v-model="queryform.open" placeholder="开启状态" style="width: 80px;" clearable @change="handleQuery()">
            <el-option label="全部" value="" />
        ...
        <el-form-item>
          <el-select v-model="queryform.status" placeholder="运行状态" style="width: 80px;" clearable @change="handleQuery()">
            <el-option label="全部" value="" />
            <el-option label="停止" value="0" />
            <el-option label="运行中" value="1" />
            <el-option label="启动中" value="2" />
            <el-option label="提交成功" value="3" />
      ...
      <el-table
        :data="list"
        :header-cell-style="{background:'#f4f4f5','text-align':'center'}"
        :row-style="{height:'20px'}"
        :cell-style="{padding:'3px 0'}"
        class="wl-table"
        border
      >
        <el-table-column prop="id" :show-overflow-tooltip="true" label="ID" min-width="50" align="center" fixed>
        ...
        <el-table-column prop="jobName" :show-overflow-tooltip="true" label="任务名称" min-width="230" align="center" fixed>
          <template slot-scope="scope">
            <i
              v-clipboard:copy="scope.row.jobName"
        ...
        <el-table-column prop="jobId" :show-overflow-tooltip="true" label="Flink任务Id" width="275" align="center">
          <template slot-scope="scope">
            <i
              ...
              v-clipboard:copy="scope.row.jobId"
              v-clipboard:success="onCopySuccess"
              v-clipboard:error="onCopyError"
              class="el-icon-copy-document"
            />
        ...
        <el-table-column :show-overflow-tooltip="true" label="版本" min-width="50" align="center">
          <template slot-scope="scope">
            <router-link :to="{name:'HistoryTask', params:{flag:'tasklist', context:queryContent(), jobConfigId:scope.row.id}}">
              <el-link type="info" class="fl-version-text">
                <span class="fl-version-span">{{ scope.row.version }}</span>
                <!-- {{ scope.row.jobDesc }} -->
              </el-link>
            </router-link>
          </template>
        ...
        <el-table-column label="日志" width="70" align="center">
          <template slot-scope="scope">
            <router-link v-if="scope.row.lastRunLogId!==null" :to="{name:'ViewTaskLogDetail', params:{flag:'tasklist', context:queryContent(), data:{id:scope.row.lastRunLogId}}}">
              <el-link type="info" icon="el-icon-message">日志</el-link>
            </router-link>
          </template>
        </el-table-column>
        <el-table-column label="SP恢复" :show-overflow-tooltip="true" align="center" width="65">
          <template slot-scope="scope">
            <el-switch
              ...
              :inactive-value="0"
              active-color="#13ce66"
              @change="changeAutoRestore($event, scope.row)"
            />
            <!-- <el-link v-if="scope.row.alarmStrs">{{ scope.row.alarmStrs }}</el-link> --><!-- 辅助 -->
          </template>
        </el-table-column>
        <el-table-column prop="savepoint" label="操作" width="105" fixed="right">
          <template slot-scope="scope">
            <el-dropdown split-button type="primary" size="mini">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item>
                  <el-link
                    v-if="scope.row.isOpen===1&&scope.row.status!=='RUN'"
                    type="success"
                ...
                <el-dropdown-item>
                  <el-link
                    ...
                    @click.native="doStopTask(scope.row)"
                  >停止</el-link>
                </el-dropdown-item>
                <el-dropdown-item>
                  <router-link :to="{name:getRouteTaskName('view',scope.row.jobTypeEnum), params:{flag:'view', context:queryContent(), data:scope.row}, query:{flag:'update', id:scope.row.id}}">
                    <el-link type="info" icon="el-icon-view">详情</el-link>
                  </router-link>
                </el-dropdown-item>
                <el-dropdown-item>
                  <router-link v-if="scope.row.isOpen===0" :to="{name:getRouteTaskName('update',scope.row.jobTypeEnum), params:{flag:'update', context:queryContent(), data:scope.row}, query:{flag:'update', id:scope.row.id}}">
                ...
                <el-dropdown-item>
                  <el-link
                    ...
                    @click.native="copyConfig(scope.row)"
                  >复制</el-link>
                </el-dropdown-item>
                <el-dropdown-item>
                  <el-link
                ...
                <el-dropdown-item divided>
                  <el-link
                    v-if="scope.row.jobTypeEnum==='SQL_STREAMING'"
                    type="primary"
                    icon="el-icon-sell"
                    @click.native="doRecoverSavePoint(scope.row)"
                  >恢复</el-link>
                </el-dropdown-item>
                <el-dropdown-item>
                  <el-link
                    ...
                    @click.native="savePoint(scope.row)"
                  >备份</el-link>
                </el-dropdown-item>
                <el-dropdown-item divided>
                  <router-link v-if="scope.row.lastRunLogId!==null" :to="{name:'ViewTaskLogDetail', params:{flag:'tasklist', context:queryContent(), data:{id:scope.row.lastRunLogId}}}">
    ...
    <!-- 恢复任务弹出窗口-->
    <recover-save-point-dialog ref="recoverSavePointDialog" />
    <!-- 停止任务执行savepoint弹出窗口 -->
    <stop-task-dialog ref="stopTaskDialog" />
    <!-- 子菜单路由 -->
    <router-view />
  </div>
</template>
...
import RecoverSavePointDialog from '@/views/flink/components/recover'
import StopTaskDialog from '@/views/flink/task-manage/stoptaskdialog'

export default {
  name: 'TaskManage',
  components: {
    RecoverSavePointDialog,
    StopTaskDialog
  },
  data() {
    return {
  ...
  methods: {
    doStopTask(row) { // 停止任务执行savepoint
      this.$refs.stopTaskDialog.openDialog(row)
    },
    onCopySuccess() {
    ...
    resetForm() {
      this.queryform.jobId = ''
      this.queryform.jobName = ''
      this.queryform.jobType = '0'
      this.queryform.open = ''
    ...
    changeAutoRestore(callback, row) { // 开启或关闭任务
      const text = (callback === 1) ? '开启' : '关闭'
      const { id, jobName, autoRestore } = row
      this.$confirm(`是否${text}[${jobName}]从savepoint恢复启动吗？`, '提示', {
        ...
        this.loading = true
        setAutoRestore(id, autoRestore).then(response => {
          this.loading = false
          const { code, success, message } = response
          if (code !== '200' || !success) {
            row.autoRestore = (callback === 1) ? 0 : 1
            this.$message({ type: 'error', message: (message || '请求数据异常！') })
            return
          }
    ...
    doRecoverSavePoint(row) { // 从SavePoint中恢复任务
      this.$refs.recoverSavePointDialog.openDialog(row)
    },
    formatDateTime(date) {
