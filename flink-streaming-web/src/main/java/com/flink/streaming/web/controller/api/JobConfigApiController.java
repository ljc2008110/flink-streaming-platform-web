...
public class JobConfigApiController extends BaseController {
    ...
    private final JobServerAO jobYarnServerAO;
    private final JobServerAO jobStandaloneServerAO;
    private final JobConfigService jobConfigService;
    private final JobConfigAO jobConfigAO;
    private final JobStatusService jobStatusService;
    private final JobConfigHistoryService jobConfigHistoryService;
    private final SystemConfigService systemConfigService;
    public final JobAlarmConfigService jobAlarmConfigService;
    private final SavepointBackupService savepointBackupService;
    private Map<DeployModeEnum, String> domainKey;

    public JobConfigApiController(JobServerAO jobYarnServerAO, JobServerAO jobStandaloneServerAO, JobConfigService jobConfigService, JobConfigAO jobConfigAO, JobStatusService jobStatusService, JobConfigHistoryService jobConfigHistoryService, SystemConfigService systemConfigService, JobAlarmConfigService jobAlarmConfigService, SavepointBackupService savepointBackupService) {
        this.jobYarnServerAO = jobYarnServerAO;
        this.jobStandaloneServerAO = jobStandaloneServerAO;
        this.jobConfigService = jobConfigService;
        this.jobConfigAO = jobConfigAO;
        this.jobStatusService = jobStatusService;
        this.jobConfigHistoryService = jobConfigHistoryService;
        this.systemConfigService = systemConfigService;
        this.jobAlarmConfigService = jobAlarmConfigService;
        this.savepointBackupService = savepointBackupService;
    }

    @PostConstruct
    public void initAfter() {
        domainKey = new HashMap<>(3);
    ...
    @RequestMapping("/stop")
    public RestResult stop(Long id, Boolean isSavePoint) {
        try {
            this.getJobServerAO(id).stop(id, this.getUserName(), isSavePoint);
        } catch (BizException e) {
    ...
    public RestResult close(Long id) {
        try {
    ...
    @RequestMapping("/setAutoRestore")
    public RestResult autoRestore(Long id, Integer autoRestore) {
        try {
            ...
            log.info("设置任务（id：{}）从sp恢复开关状态为：{}", id, autoRestore);
        } catch (BizException e) {
    ...
    public RestResult editConfig(UpsertJobConfigParam upsertJobConfigParam) {
        try {
            ...
            completeJobConfigDTO(jobConfigDTO, list);
            ...
            if (!checkJobConfigChanged(jobConfigDTO, jobConfigNew)) {
                log.info("任务ID：{}，任务名称：{}，没有产生任何修改，不需要保存！", jobConfigDTO.getId(), jobConfigDTO.getJobName());
                return RestResult.success("没有产生任何修改，不需要保存！");
            }
    ...
    public RestResult getTask(ModelMap modelMap, Long id) {
        if (id==null){
            return RestResult.error("参数错误，未找到对应任务！");
        }
        JobConfigDTO jobConfigDTO = jobConfigService.getJobConfigById(id);
        List<AlarmTypeEnum> list = jobAlarmConfigService.findByJobId(jobConfigDTO.getId());
        completeJobConfigDTO(jobConfigDTO, list);
        JobConfigVO jobConfigVO = JobConfigVO.toVO(jobConfigDTO, domainKey);
        return RestResult.success(jobConfigVO);
    }

    /**
     * 查询作业列表
     *
     * @param jobConfigParam 任务配置信息
     * @return 返回任务配置列表页
     * @author wxj
     * @date 2021年12月1日 下午5:09:06
     * @version V1.0
    ...
    public RestResult listTask(JobConfigParam jobConfigParam) {
        ...
        PageVO pageVO = convertToVOFromModel(pageModel);
    ...
    private PageVO convertToVOFromModel(PageModel<?> pageModel) {
    ...
    private RestResult checkUpsertJobConfigParam(UpsertJobConfigParam upsertJobConfigParam) {
        ...
        if (DeployModeEnum.YARN_PER.name().equals(upsertJobConfigParam.getDeployMode())) {
            ...
            return CliConfigUtil.checkFlinkRunConfigForYarn(upsertJobConfigParam.getFlinkRunConfig());
        }
     ...
     * 判断任务内容是否修改
     *
     * @param oldJob 旧任务配置信息
     * @param newJob 新任务配置信息
     * @return 返回是否有修改
     * @author wxj
     ...
     * 补充字段信息
     *
     * @param jobConfigDTO 补充字段
     * @author wxj
     * @date 2021年12月21日 下午5:01:47
     * @version V1.0
     */
    private void completeJobConfigDTO(JobConfigDTO jobConfigDTO, List<AlarmTypeEnum> list) {
        // 补充FlinkRunUrl字段
    ...
    private void completeJobConfigDTO(PageModel<JobConfigDTO> pageModel) {
        ...
        for (JobConfigDTO jobConfigDTO : pageModel) {
            completeJobConfigDTO(jobConfigDTO, map.get(jobConfigDTO.getId()));
        }
    }
}
