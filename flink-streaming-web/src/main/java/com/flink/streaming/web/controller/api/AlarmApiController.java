...
public class AlarmApiController extends BaseController {
    ...
    public RestResult<?> queryalarmLogList(ModelMap modelMap, AlarmLogParam alarmLogParam) {
        ...
        PageVO<PageModel<AlarmLogDTO>> pageVO = new PageVO<PageModel<AlarmLogDTO>>();
    ...
    public RestResult<?> alarmConfig(ModelMap modelMap) {
    ...
    public RestResult testDingdingAlert() {
        try {
            String alarmUrl = systemConfigService.getSystemConfigByKey(SysConfigEnum.DINGDING_ALARM_URL.getKey());
            if (StringUtils.isEmpty(alarmUrl)) {
                return RestResult.error("钉钉告警地址不存在");
            }
            boolean isSuccess = alarmServiceAO.sendForDingding(alarmUrl,
